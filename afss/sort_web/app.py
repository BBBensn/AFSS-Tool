...
from afss.tagging import search_entities
    ...
    def bulk(profile_id: str):
        ...
        if action == "set_artist":
            ...
            if not target_id:
                ...
                n = bulk_update(item_ids, {"artist_id": target_id}, db_path)
            ...
            if not target_id:
                ...
                n = bulk_update(item_ids, {"provider_id": target_id}, db_path)
                flash(f"{n} Datei(en) neu zugeordnet.", "ok")
        elif action == "set_collection":
            collection_name = request.form.get("collection_name", "").strip() or None
            n = bulk_update(item_ids, {"collection_name": collection_name}, db_path)
